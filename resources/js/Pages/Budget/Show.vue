<template>
    ...
    <v-card class="mt-2">
        <v-tabs v-model="tab" bg-color="light-green" density="comfortable">
            ...
            <v-tab v-if="shareTabVisible" value="two">
                {{ shareUserName }}
            </v-tab>
        </v-tabs>
    </v-card>
    ...
    <div>
        <v-window v-model="tab">
            ...
            <v-window-item v-if="shareTabVisible" value="two">
...
import { useI18n } from 'vue-i18n'
    ...
    const shareTabVisible = computed(() => Boolean(props.shareUser && budgetShareId.value))
    ...
    const budgetExpenseToTagOptions = computed(() => props.owner.budget.expenseTagOptions ?? [])
    const budgetExpenseToTagOptionCharts = computed(() => props.owner.expenseToTagOptionCharts ?? [])
    ...
    const budgetShareExpenseToTagOptions = computed(() => props.share?.budget?.expenseTagOptions ?? [])
    const budgetShareExpenseToTagOptionCharts = computed(() => props.share?.expenseToTagOptionCharts ?? [])
