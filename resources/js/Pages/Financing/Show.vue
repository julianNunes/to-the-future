<template>
        ...
        <v-card-text class="pa-4">
            <v-row dense>
                <v-col cols="12" sm="12" md="12">
                    <v-text-field v-model="financingDescription" :label="$t('default.description')" :readonly="true"
                        density="comfortable"></v-text-field>
                </v-col>
                <v-col cols="12" sm="6" md="3">
                    <v-text-field v-model="financingStartDate" :label="$t('financing.start-date')" :readonly="true"
                        density="comfortable"></v-text-field>
                </v-col>
                <v-col cols="12" sm="6" md="3">
                    <v-text-field v-model="financingTotal" :label="$t('default.total')" :readonly="true"
                        density="comfortable"></v-text-field>
                </v-col>
                <v-col cols="12" sm="6" md="3">
                    <v-text-field v-model="financingFeesMonthly" :label="$t('financing.fees-monthly')" :readonly="true"
                        density="comfortable"></v-text-field>
                </v-col>
                <v-col cols="12" sm="6" md="3">
                    <v-text-field v-model="financingPortionTotal" :label="$t('financing.portion-total')"
                        :readonly="true" density="comfortable"></v-text-field>
                </v-col>
                <v-col cols="12" md="12">
                    <v-text-field v-model="financingRemarks" :label="$t('default.remarks')" :readonly="true"
                        density="comfortable"></v-text-field>
                </v-col>
            </v-row>
        </v-card-text>
    ...
    <v-card class="mt-4">
        ...
        <v-card-text class="pa-4">
            <v-row dense>
                <v-col md="12">
                    <v-data-table :headers="headers" :items="installments" :sort-by="[{ key: 'portion', order: 'asc' }]"
                        :search="search" :loading="isLoading" :loading-text="$t('default.loading-text-table')"
                        class="elevation-3" density="compact" :total-items="installments.length"
                        ...
                        <template #[`item.action`]="{ item }">
                            <v-tooltip :text="$t('default.edit')" location="top">
                                <template #activator="{ props }">
                                    <v-icon v-bind="props" color="warning" icon="mdi-pencil" size="small" class="me-2"
                                        @click="editItem(item)">
                                    </v-icon>
                                </template>
                            </v-tooltip>
        ...
        <v-card>
            ...
            <v-card-text>
                <v-form ref="form" @submit.prevent>
                    <v-row dense>
                        <v-col cols="12" sm="6" md="3">
                            <v-text-field v-model="installment.portion" :label="$t('financing-installment.portion')"
                                :readonly="true" density="comfortable"></v-text-field>
                        </v-col>
                        <v-col cols="12" sm="6" md="3">
                            <v-date-input v-model="installment.date" :label="$t('default.date')" prepend-icon=""
                                prepend-inner-icon="$calendar" required :rules="rules.textFieldRules"
                        ...
                        <v-col cols="12" sm="6" md="3">
                            <vuetify-money v-model="installment.value" :label="$t('default.value')"
                        ...
                        <v-col cols="12" sm="6" md="3">
                            <v-select v-model="installment.paid" label="Status" :items="listStatus" item-title="name"
                                item-value="value" clearable :rules="[
                                    (value) => {
                        ...
                        <v-col cols="12" sm="6" md="3">
                            <v-date-input v-model="installment.payment_date"
                                :label="$t('financing-installment.payment-date')" prepend-icon=""
                                prepend-inner-icon="$calendar" required density="comfortable" :rules="[
                                    (value) => {
                                ...
                                ]" :show-adjacent-months="true" :show-week="true"
                                :display-format="(date) => formatDate(date, 'DD/MM/YYYY')" placeholder="DD/MM/YYYY"
                                :update-on="['enter']"></v-date-input>
                        </v-col>
                        <v-col cols="12" sm="6" md="3">
                            <v-text-field v-model="installment.paid_value" type="number"
                                :label="$t('financing-installment.paid-value')" min="0" required :rules="[
                                    (value) => {
                                        if (installment.paid == 1) {
                                            if (!value) return $t('rules.required-text-field')
