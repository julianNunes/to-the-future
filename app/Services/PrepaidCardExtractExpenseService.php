...
use App\Repositories\Interfaces\{
    PrepaidCardExtractExpenseDivisionRepositoryInterface,
    PrepaidCardExtractExpenseRepositoryInterface,
    PrepaidCardExtractFileRepositoryInterface,
...
class PrepaidCardExtractExpenseService implements PrepaidCardExtractExpenseServiceInterface
{
    ...
    public function createWithPortions(
        ...
        $credit_card_invoice = $this->prepaidCardExtractRepository->show($invoiceId);
        ...
        if ($portionTotal && $portionTotal > 1) {
            ...
            for ($i = ($portion + 1); $i <= $portionTotal; $i++) {
                ...
                $new_credit_card_invoice = $this->prepaidCardExtractRepository->getOne(['year' => $new_due_date->year, 'month' => $new_due_date->month, 'credit_card_id' => $creditCardId]);

                // Se não existe, ela é criada
                if (!$new_credit_card_invoice) {
                    $new_credit_card_invoice = $this->prepaidCardExtractRepository->store([
    ...
    public function create(
        ...
        $credit_card_invoice_expense = $this->prepaidCardExtractExpenseRepository->store([
    ...
    public function update(
        ...
        $credit_card_invoice_expense = $this->prepaidCardExtractExpenseRepository->show($id);
    ...
    public function delete(int $id): bool
    {
        ...
        $this->prepaidCardExtractExpenseRepository->delete($credit_card_invoice_expense->id);
    ...
    {
        $credit_card_invoice_expense = $this->prepaidCardExtractExpenseRepository->show($id, ['invoice']);
        ...
        $expenses = $this->prepaidCardExtractExpenseRepository->get(function (Builder $query) use ($credit_card_id, $portion_total, $description) {
    ...
    public function recalculateTotalInvoice(int $invoiceId)
    {
        ...
        // Atualiza o saldo total da fatura
        $expenses = $this->prepaidCardExtractExpenseRepository->get(['invoice_id' => $invoiceId]);
        ...
        if ($expenses && $expenses->count()) {
            ...
            $this->prepaidCardExtractRepository->store(['total' => $total], $credit_card_invoice);
        }
    }
}
